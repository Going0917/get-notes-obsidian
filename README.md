# Get 笔记 → Obsidian 同步工具

> 将 [Get 笔记](https://www.biji.com/)（罗辑思维旗下 AI 笔记应用）的内容自动同步到 Obsidian Vault，按主题分类保存为标准 Markdown 文件。

---

## 功能特性

- **增量同步**：只拉取上次同步后新增的笔记，避免重复处理
- **全量同步**：一键重新同步所有历史笔记（`--full-sync`）
- **自动类型识别**：
  - 🎙 `播客笔记` — 小宇宙、喜马拉雅、Spotify 等平台
  - 🎤 `语音备忘` — 个人录音笔记
  - 📰 `文章剪藏` — 网页、小红书等链接收藏
  - 📚 `读书笔记` — 书摘、阅读笔记
  - 💼 `工作笔记` — 含工作相关标签的内容
  - 📄 `其他笔记` — 无法识别类型时的兜底分类
- **结构化输出**：每篇笔记包含 YAML frontmatter、AI 总结、章节、金句等结构化内容；工作会议保留原文转写，语音日记只保留整理后的笔记内容
- **智能 Token 管理**：首次登录后自动刷新，约 90 天内无需重新登录
- **全局防重复保护**：写入前扫描整个 `Get笔记沉淀` 目录中的 `id: get-...`，避免目录规则变化后重复写入
- **干净文件名**：新增笔记使用短日期 `YY.MM.DD_标题.md`，Get ID 只保存在 frontmatter 中，不污染 Obsidian 文件列表
- **稳定分类规则**：工作笔记按客户/业务主题/内部复盘归档，英语学习独立成 `09_英语学习`，外部播客不再混入职场目录
- **工作区副本规则**：工作笔记主副本先进入 `Get笔记沉淀`，同步完成并经用户确认后，才按规则创建 `Working💻` 副本；已有副本按 Get ID、标题和日期跳过

---

## 输出目录结构

```
Going Knowledge/
└── Get笔记沉淀/
    ├── 00_其他/
    ├── 01_AI与科技/
    ├── 02_职场工作/
    │   ├── 01_客户沟通复盘/
    │   │   ├── 华大营养/
    │   │   ├── 诺特兰德/
    │   │   ├── 星盟/
    │   │   ├── 万益蓝/
    │   │   ├── 养元健/
    │   │   ├── 多燕瘦/
    │   │   ├── Swisse/
    │   │   └── 谷雨/
    │   ├── 02_业务主题研究/
    │   │   ├── 视频号小店与直播电商/
    │   │   ├── 投放与广告策略/
    │   │   ├── 私域与公私域联动/
    │   │   ├── 内容力与商品力/
    │   │   └── 大健康与营养品类/
    │   ├── 03_内部汇报与项目复盘/
    │   │   ├── 晋升答辩/
    │   │   ├── OKR与业务规划/
    │   │   └── 内部分享/
    │   ├── 04_AI工具与业务提效/
    │   └── 99_待归类/
    ├── 03_财商投资/
    ├── 04_自我成长/
    │   ├── 01_认知与表达/
    │   ├── 02_心理与情绪/
    │   ├── 03_职业心态与成长/
    │   └── 04_关系与沟通/
    ├── 05_旅行/
    │   ├── 日本/
    │   ├── 东南亚/
    │   └── 国内/
    ├── 06_生活/
    │   ├── 消费选品/
    │   └── 运动健康/
    ├── 07_语音日记/
    │   └── YYYY-MM/
    ├── 08_读书笔记/
    └── 09_英语学习/
        ├── 学习计划/
        └── 口语场景/
```

文件名规则：

- 通用笔记：`YY.MM.DD_标题.md`
- 语音日记：`YY.MM.DD_HHMM_语音日记.md`
- Get 笔记 ID 只写入 frontmatter：`id: get-...`

同步日志规则：

- `00-同步日志.md` 使用表格记录 `日期 | 标题 | 类型 | 位置`。
- 日期统一使用短日期 `YY.MM.DD`。
- 记录按时间倒序排列，最新笔记在最上方。
- 标题尽量保留完整，并使用 Obsidian wikilink 指向实际文件。
- 位置填写笔记所在目录，便于快速检索。

内容规则：

- 工作笔记：如果 Get 笔记提供原文转写，底部保留 `## 🎙 原文转写`。
- 语音日记：只保留 `## 📝 笔记内容`，不输出录音原文、AI 整理或摘录。
- 播客/文章：继续按原模板保留平台返回的结构化内容和原文。

Working 工作区副本规则：

- `Working💻` 不是主沉淀目录，不在 `sync.py` 的新增同步过程中直接自动写入。
- 每次新增同步完成后，如本次包含工作笔记，先向用户列出建议创建的 Working 副本清单。
- 用户确认无误后，再创建副本；未确认前不写入 `Working💻`。
- 跳过重复时优先读取目标工作区已有文件的 frontmatter `id: get-...`。
- 如果已有手动整理版没有 Get ID，则按日期、标题关键词、客户/主题目录做近似匹配，确认疑似重复时跳过。
- 客户沟通类进入 `Working💻/客户沟通会议/{客户}/`。
- 与中台协同相关的公私域、私域投流、营销产品共建、项目推进和后续沟通进入 `Working💻/06-with中台/公私域协同/`。
- 风控审核、功效标签、类目规则类进入 `Working💻/03-赛道行业/with 风控/` 或相关赛道目录。
- 非实际工作会议的外部内容、平台文章和工具教程按主分类归档，不创建 Working 工作副本。

---

## 环境要求

- Python 3.9+
- [Get 笔记](https://www.biji.com/) 账号（已有笔记内容）
- [Obsidian](https://obsidian.md/) 已安装并创建 Vault

---

## 安装

```bash
# 1. 克隆项目
git clone https://github.com/Going0917/get-notes-obsidian.git
cd get-notes-obsidian

# 2. 安装依赖
pip install -r requirements.txt

# 3. 安装 Playwright 浏览器（首次登录需要）
playwright install chromium

# 4. 配置环境变量
cp .env.example .env
# 编辑 .env，填写你的 Going Knowledge/Get笔记沉淀 路径
```

---

## 配置

编辑 `.env` 文件：

```ini
# 必填：Going Knowledge 中用于存放 Get 笔记的目录
OBSIDIAN_VAULT_PATH=/Users/your-name/Library/Mobile Documents/iCloud~md~obsidian/Documents/Going Knowledge/Get笔记沉淀

# 可选：每次 API 分页大小（默认 50）
# SYNC_LIMIT=50
```

> ⚠️ `.env` 文件已被 `.gitignore` 排除，不会提交到 Git。

---

## 使用方法

### 增量同步（推荐日常使用）

```bash
python3 sync.py
```

### 其他模式

```bash
# 预览有哪些新笔记（不写文件）
python3 sync.py --dry-run

# 全量重新同步所有笔记
python3 sync.py --full-sync

# 只同步指定类型
python3 sync.py --type podcast    # 只同步播客
python3 sync.py --type voice      # 只同步语音备忘
python3 sync.py --type article    # 只同步文章剪藏
python3 sync.py --type book       # 只同步读书笔记
python3 sync.py --type work       # 只同步工作笔记

# 限制本次同步数量
python3 sync.py --limit 10
```

### 首次运行

首次运行时，程序会自动打开浏览器，引导你完成 Get 笔记登录：

1. 浏览器打开后，点击登录（支持手机扫码或账号密码）
2. 登录成功、看到笔记列表后，回到终端按 **Enter** 继续
3. Token 会自动保存到 `.tokens/tokens.json`，后续运行无需重复登录

---

## 同步输出示例

```
============================================================
📥 Get 笔记同步工具
============================================================
📂 输出目录：/Users/your-name/Library/Mobile Documents/iCloud~md~obsidian/Documents/Going Knowledge/Get笔记沉淀
📊 历史累计同步 0 条，上次同步时间：从未同步
🔍 开始增量拉取（上次位置：无，首次全量）
📋 找到 12 条新笔记，开始拉取详情...

🚀 开始同步 12 条笔记...

  ✅ [1/12] podcast  | 2026-03-10 | AI 时代的认知革命
  ✅ [2/12] voice    | 2026-03-11 | 26.03.11_1435_语音日记.md
  ✅ [3/12] article  | 2026-03-12 | 如何构建第二大脑
  ...

============================================================
📋 同步完成
   本次同步：12 条
   类型分布：播客×5 | 文章×4 | 语音备忘×2 | 读书笔记×1
   最新笔记：2026-03-15 ｜ AI 时代的认知革命
   输出目录：/Users/your-name/Library/Mobile Documents/iCloud~md~obsidian/Documents/Going Knowledge/Get笔记沉淀
============================================================
```

---

## 笔记文件格式

每篇笔记包含完整的 YAML frontmatter 和结构化内容。

**播客笔记** 会同时保存 AI 结构化笔记和原文逐字转写：

```markdown
---
id: get-1904618693187502944
type: podcast
source: "小宇宙"
episode: "AI 时代的认知革命"
url: https://www.xiaoyuzhoufm.com/episode/...
date: 2026-03-15
tags: [podcast, ai, 认知]
synced_at: 2026-03-18T12:00:00+00:00
---

# AI 时代的认知革命

> 来源：[小宇宙](https://...)｜时长：52min｜2026-03-15

---

**目录：** [📝 AI 总结](#-ai-总结) · [🏷 重点章节](#-重点章节) · [💬 金句摘录](#-金句摘录) · [📄 AI 笔记](#-ai-笔记) · [📃 原文转写](#-原文转写)

## 📝 AI 总结

...（如平台提供）

## 🏷 重点章节

### 00:15:44 — 什么是认知革命
...（如平台提供）

## 💬 金句摘录

> "在 AI 时代，脑子里的想法才是核心。"（如平台提供）

## 📄 AI 笔记

...（Get 笔记 AI 生成的结构化笔记，Markdown 格式）

## 📃 原文转写

[00:00:00] 大家好，欢迎收听本期节目...
[00:00:05] 今天我们来聊聊...
...（完整逐字转写，带时间戳）
```

> 说明：`📝 AI 总结`、`🏷 重点章节`、`💬 金句摘录` 仅在 Get 笔记平台返回对应字段时出现；`📄 AI 笔记` 和 `📃 原文转写` 为主要内容，播客类笔记通常都有。

---

## 自定义分类

### 添加工作笔记关键词

在 `get_notes/parser.py` 中修改 `_WORK_TAGS`：

```python
_WORK_TAGS = {"工作", "work", "职场", "项目", "okr",
              # 添加你自己的公司/行业词汇：
              "腾讯", "华大营养", "诺特兰德", "视频号", "私域", ...}
```

### 调整工作目录路由

在 `get_notes/renderer.py` 中修改：

- `_WORK_BRAND_RULES`：客户/品牌目录，例如华大营养、诺特兰德、星盟、万益蓝。
- `_WORK_TOPIC_RULES`：业务主题目录，例如视频号小店与直播电商、投放与广告策略、私域与公私域联动。
- `_GROWTH_RULES` / `_LIFE_RULES`：外部播客和个人内容的非工作分类。

### 添加播客平台

在 `get_notes/parser.py` 中修改 `_PODCAST_URL_DOMAINS`（URL 匹配，精确）或 `_PODCAST_PLATFORMS`（名称匹配，宽松）。

### 屏蔽不想同步的笔记

在 `get_notes/parser.py` 中修改 `BLOCKED_NOTE_IDS`：

```python
BLOCKED_NOTE_IDS: set[str] = {
    "1894909191618305632",   # 不想要的笔记 ID
}
```

> 提示：运行 `python3 sync.py --dry-run` 可以看到所有笔记的 ID 信息。

---

## 项目结构

```
get-notes-obsidian/
├── .env.example          # 配置示例
├── .gitignore
├── requirements.txt
├── sync.py               # 主入口
└── get_notes/
    ├── auth.py           # Token 获取/刷新（Playwright 登录）
    ├── client.py         # API 请求封装
    ├── config.py         # 配置管理
    ├── fetcher.py        # 分页拉取 + 增量控制
    ├── parser.py         # 笔记类型识别 + 内容结构化
    ├── renderer.py       # Obsidian Markdown 渲染
    └── state.py          # 增量同步状态管理
```

---

## 常见问题

**Q: 首次运行时浏览器打开后我应该做什么？**

登录 Get 笔记（biji.com）即可，看到笔记列表后回到终端按 Enter。程序会自动提取登录凭证并保存。

**Q: Token 多久过期？**

JWT token 有效期约 1 小时，程序会自动刷新。Refresh Token 有效期约 90 天，无需手动操作。90 天后再次运行时会重新打开浏览器登录。

**Q: 如何重置并全量重新同步？**

```bash
rm -f .sync_state.json
python3 sync.py --full-sync
```

**Q: 已同步的笔记内容有更新，能重新同步吗？**

目前同步器会按 frontmatter 的 `id: get-...` 全局去重，已存在同 ID 文件时不会覆盖。如需重新同步某篇，先备份并移走对应 `.md` 文件，再重新运行同步。

**Q: 支持 Windows 吗？**

理论上支持（Python + Playwright 均跨平台），但目前仅在 macOS 上测试过。欢迎提 Issue 反馈。

---

## 致谢

- [Get 笔记 / biji.com](https://www.biji.com/) — 数据来源
- [Obsidian](https://obsidian.md/) — 知识管理工具
- [Playwright](https://playwright.dev/) — 浏览器自动化登录

---

## License

MIT
